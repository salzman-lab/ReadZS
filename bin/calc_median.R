...
all_pvals_table[, significant := ifelse(pval <= largest_pval, TRUE, FALSE)]  # only significant if p-value is less than largest p-value allowed
...
essential_cols <- c("window", "ontology", "sum_counts_per_window_per_ont", "med_counts_per_window_per_ont", "median_z_scaled", "pval", "significant")
